...
class REP:
    ...
    def generate_protein_pretraining_representation(self, prots):
                ...
                prots_tuple = [(str(i), prots) for i in range(len(prots))]
                ...
                while (batch*i) < len(prots):
                    ...
                    batch_labels, batch_strs, batch_tokens = self.batch_converter(pt)
...
class DataProvider:
    ...
    def read_hla_sequences(self):
        ...
        file_path = os.path.join(BASE_DIR, '../..','dataset/Anthem_dataset',  'proteins_esm.txt')
    ...
    def batch(self, batch_index, sample_set, testing=False):
        ...
        def encode_sample(sample):
            ...
            if hla_a_allele not in self.hla_encode_dict:
                hla_a_tensor = rep.generate_protein_pretraining_representation(self.hla_sequence[hla_a_allele])
